...
const PositionTable: FC<PositionTableProps> = ({ currency, exchangeRate, tokenList }) => {
  const theme = useTheme()
  const upSm = useMediaQuery(theme.breakpoints.up("sm"));
  const upMd = useMediaQuery(theme.breakpoints.up("md"));
  const currencySymbol = currencies[currency]
  const filteredTokens = tokenList.filter((item) => !item.name.includes(' (Staked)') && !item.name.includes('_LP'))
